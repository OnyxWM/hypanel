...
class MockDownloader {
    static async mockInstall(serverId, serverRoot) {
...
class SmokeTestSuite {
    ...
    `);
        const now = Date.now();
        stmt.run("test-sqlite-server", "Test SQLite Server", "offline", "127.0.0.1", 25565, "1.0.0", now, now, "NOT_INSTALLED", 20, 1024);
        // Test server retrieval
        const retrieveStmt = testDb.prepare("SELECT * FROM servers WHERE id = ?");
        const retrieved = retrieveStmt.get("test-sqlite-server");
        if (!retrieved) {
        ...
        stmt.run("test-sqlite-server-2", "Test SQLite Server 2", "offline", "127.0.0.1", 25566, "1.0.0", now, now, "NOT_INSTALLED", 20, 1024);
        const allStmt = testDb.prepare("SELECT * FROM servers ORDER BY created_at DESC");
        const allServers = allStmt.all();
    ...
    `);
        const now = Date.now();
        createStmt.run("test-install-server", "Test Install Server", "offline", "127.0.0.1", 25566, "1.0.0", now, now, "NOT_INSTALLED", 20, 1024);
        // Test installation locking
        const lockCheckStmt = testDb.prepare("SELECT install_state FROM servers WHERE id = ?");
        const lockResult = lockCheckStmt.get("test-install-server");
        if (!lockResult) {
    ...
    `);
        ...
        const server = lockCheckStmt.get("test-install-server");
    ...
    `);
        ...
        const failedServer = lockCheckStmt.get("test-install-server");
        ...
        const mockInstaller = {
            on: (event, callback) => {
                if (event === "progress") {
                    ...
                    mockInstaller.progressCallback = callback;
                }
            },
            emit: (event, serverId, progress) => {
                if (event === "progress" && mockInstaller.progressCallback) {
                    mockInstaller.progressCallback(serverId, progress);
                }
            }
        };
        mockInstaller.on("progress", (serverId, progress) => {
    ...
    `);
        const now = Date.now();
        createStmt.run("test-full-workflow", "Test Full Workflow", "offline", "127.0.0.1", 25567, "1.0.0", now, now, "NOT_INSTALLED", 20, 1024);
      ...
      `);
        ...
        const finalState = checkStmt.get("test-full-workflow");
