 ...
 */
async function computeSystemStats(serverManager) {
...
function startSystemStatsRecording(serverManager) {
...
export function createSystemRoutes(serverManager) {
    ...
    router.get("/stats/history", (req, res) => {
        try {
            const window = req.query.window || "12h";
            const windowMs = {
                "1h": 60 * 60 * 1000,
                "12h": 12 * 60 * 60 * 1000,
                "24h": 24 * 60 * 60 * 1000,
            ...
            try {
                ...
                if (filterBeta) {
                    ...
                    const betaReleases = releases
                        ...
                        return tagName.endsWith("-beta") && !release.draft;
                    })
                        .sort((a, b) => {
        ...
        try {
            ...
            try {
                ...
                if (serversToStop.length > 0) {
                    await Promise.allSettled(serversToStop.map(async (id) => {
                        try {
            ...
            const downloadUrl = latestRelease.assets?.find((asset) => asset.name?.endsWith(".tar.gz"))?.browser_download_url;
            if (!downloadUrl) {
                return res.status(500).json({
                    success: false,
