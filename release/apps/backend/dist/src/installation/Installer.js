...
export class Installer extends EventEmitter {
    ...
    async installServer(serverId) {
        ...
        if (!lockResult.success) {
            const error = createInstallationError("locking", lockResult.reason || "Failed to acquire installation lock", serverId, "Check if another installation is already running for this server");
        ...
        try {
            ...
            if (process.env.NODE_ENV === "production") {
                ...
                if (!downloaderPath) {
                    const error = createInstallationError("downloader_check", "hytale-downloader not found", serverId, "Run the install.sh script to install hytale-downloader, or install it manually and ensure it's in PATH");
                    logError(error, "install", serverId);
                    throw error;
                }
            ...
            if (isDev) {
                ...
                if (!downloaderPath) {
                    ...
                    downloadResult = await this.executeDownloader(downloaderPath, server.serverRoot, serverId);
                }
            }
            ...
            if (!verification.valid) {
                ...
                const error = createInstallationError("verification", `${verification.error || "Required files not found"}${downloaderOutput}${downloaderErrors}`, serverId, "Reinstall server to ensure all required files are downloaded");
                logError(error, "install", serverId);
                throw error;
            }
    ...
    async mockInstallation(serverRoot, serverId) {
    ...
    async verifyAndFixPermissions(serverRoot, serverId) {
    ...
    async executeDownloader(downloaderPath, serverRoot, serverId) {
        return new Promise((resolve) => {
            const args = [
    ...
    async extractDownloadedZip(serverRoot, serverId) {
        ...
        try {
            ...
            try {
                ...
                throw createInstallationError("extraction", `Failed to extract downloaded ZIP file: ${errorMessage}`, serverId, "Check disk space and permissions, then retry installation");
            }
    ...
    parseProgressFromOutput(serverId, output) {
    ...
    async findFileRecursively(rootDir, filename, maxDepth = 3) {
    ...
    async verifyInstallation(serverRoot) {
        try {
    ...
    async updateInstallState(serverId, state, error, jarPath, assetsPath) {
        try {
    ...
    emitProgress(serverId, progress) {
    ...
    async recoverInterruptedInstallations() {
        ...
        try {
            ...
            if (interruptedServers.length > 0) {
                ...
                for (const server of interruptedServers) {
                    await this.updateInstallState(server.id, "FAILED", "Installation was interrupted due to daemon restart. Please retry the installation.", null, null);
    ...
    async cleanupInstallation(serverId, serverRoot) {
