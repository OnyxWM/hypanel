...
import {
  ...
  updateServerPaths,
  updateServerConfig,
  updateServerInstallState,
  setLastScheduledRestartAt,
  setLastRestartWarningForRunAt,
  setLastAdvancedBackupAt,
  insertNotification,
  pruneNotifications,
} from "../database/db.js";
...
export class ServerManager extends EventEmitter {
  ...
  private advancedBackupInterval: NodeJS.Timeout | null = null;
      ...
      try {
        ...
        for (const s of targets) {
          try {
            ...
            logger.warn(
              `Autostart: failed to start server ${s.id} (${s.name}): ${err instanceof Error ? err.message : String(err)}`
            );
          }
        }
      } catch (err) {
  ...
  private getHytaleConfigMaxPlayers(serverId: string): number | undefined {
    try {
      const dbServer = getServerFromDb(serverId);
      if (!dbServer) return undefined;
  ...
  private notify(input: {
    ...
    try {
      ...
      logger.warn(
        `Failed to record notification (${input.type}) for server ${input.serverId || "n/a"}: ${
  ...
  async createServer(config: Omit<ServerConfig, "id">): Promise<Server> {
    ...
    createServerInDb({
      ...
      autostart: Boolean((config as any).autostart),
    });
   ...
   */
  async importFromHypanelBackup(
    config: Omit<ServerConfig, "id">,
    backupTarPath: string,
    overrides?: { name?: string; port?: number; maxMemory?: number }
  ): Promise<Server> {
    const server = await this.createServer(config);
    const id = server.id;
    const serverRoot = path.join(appConfig.serversDir, id);
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        const diskConfig = JSON.parse(content) as Record<string, unknown>;
        const name = (overrides?.name ?? diskConfig.name ?? config.name) as string;
      ...
      if (instance) {
        ...
        if (overrides.name) updates.name = overrides.name;
        if (overrides.port !== undefined) updates.port = overrides.port;
        if (overrides.maxMemory !== undefined) updates.maxMemory = overrides.maxMemory;
  ...
  async updateServerConfig(id: string, config: Partial<{
    ...
    try {
      ...
      const {
        ...
        restartScheduleEnabled: _re,
        restartFrequency: _rf,
        restartTime: _rt,
        restartDayOfWeek: _rd,
        advancedBackupEnabled: _abe,
        advancedBackupFrequency: _abf,
        advancedBackupTime: _abt,
        advancedBackupDayOfWeek: _abd,
        advancedBackupMaxCount: _abm,
        ...fsConfig
      } = config;
  ...
  async startServer(id: string): Promise<void> {
    ...
    try {
      ...
      this.notify({
        ...
        message: `Server "${dbServer?.name || id}" failed to start: ${
          error instanceof Error ? error.message : String(error)
        }`,
        serverId: id,
        serverName: dbServer?.name,
  ...
  async stopServer(id: string, force: boolean = false): Promise<void> {
    ...
    try {
      ...
      this.notify({
        ...
        message: `Server "${dbServer?.name || id}" failed to stop: ${
          error instanceof Error ? error.message : String(error)
        }`,
        serverId: id,
        serverName: dbServer?.name,
  ...
  async restartServer(id: string): Promise<void> {
    ...
    try {
      ...
      this.notify({
        ...
        message: `Server "${dbServer?.name || id}" failed to restart: ${
          error instanceof Error ? error.message : String(error)
        }`,
        serverId: id,
        serverName: dbServer?.name,
  ...
  async installServer(id: string): Promise<void> {
    ...
    try {
      ...
      this.notify({
        ...
        message: `Installation failed for "${dbServer?.name || id}": ${
          error instanceof Error ? error.message : String(error)
        }`,
        serverId: id,
        serverName: dbServer?.name,
  ...
  private getEffectiveStartupArgs(config: ServerConfig, dbServer: { jarPath?: string | null; assetsPath?: string | null }, serverId: string): string[] {
   ...
   */
  private parseXmxMB(args: string[]): number | null {
    ...
    if (count !== 1 || !lastMatch || lastMatch[1] === undefined || lastMatch[2] === undefined) return null;
  ...
  getServer(id: string): Server | null {
    ...
    let effectiveStartupArgs: string[] = [];
    let configForArgs: ServerConfig | null = null;
    if (!instance) {
      try {
        const serverRoot = dbServer.serverRoot && typeof dbServer.serverRoot === "string" && dbServer.serverRoot.trim() !== ""
          ? dbServer.serverRoot
    ...
    if (config.customStartupArgs && config.customStartupArgs.length > 0) {
      const xmxMB = this.parseXmxMB(effectiveStartupArgs);
      if (xmxMB !== null) displayMaxMemory = xmxMB;
    }
   ...
   */
  private async getUniqueServerDirectory(
    baseDir: string,
  ...
  updateHytaleServerConfig(id: string, updates: any): any {
    ...
    if (instance && instance.getStatus() === "online") {
      const error = createConfigError(
        ...
        id,
        "Stop the server before modifying configuration"
      );
      logError(error, "config", id);
      throw error;
    }
    ...
    if (!fs.existsSync(configPath)) {
      const error = createConfigError(
        "write",
        `Server config.json does not exist`,
        id,
        "Verify the server has been initialized"
      );
      logError(error, "config", id);
      throw error;
    }
  ...
  getBackups(): Array<{ 
    serverId: string; 
    serverName: string; 
    backups: Array<{ name: string; path: string; size: number; modified: Date; isDirectory: boolean; backupType?: "official" | "advanced" }> 
  }> {
    const serverBackups: Map<string, { serverId: string; serverName: string; backups: Array<{ name: string; path: string; size: number; modified: Date; isDirectory: boolean; backupType?: "official" | "advanced" }> }> = new Map();
    const backupDir = appConfig.backupDir;
    ...
    const ensureServerEntry = (serverId: string, serverName: string) => {
   ...
   */
  private resolveBackupPath(serverId: string, backupName: string): string {
    const backupDir = appConfig.backupDir;
  ...
  async checkServerUpdate(serverId: string): Promise<{ updateAvailable: boolean, currentVersion: string, latestVersion: string }> {
    ...
    if (!downloaderPath) {
      throw new HypanelError(
        ...
        "Ensure hytale-downloader is installed and accessible",
        undefined,
        500
      );
    }
    ...
    try {
      ...
      if (errorMessage.includes("timeout") || errorMessage.includes("timed out")) {
        throw new HypanelError(
          ...
          "Update check timed out - hytale-downloader did not respond in time. The command may be waiting for network access or authentication.",
          `Test the command manually: "${downloaderPath}" -print-version. If it hangs, check network connectivity, authentication, or hytale-downloader configuration.`,
          undefined,
          500
        );
      }
  ...
  async updateServer(serverId: string): Promise<void> {
    ...
    if (wasRunning && instance) {
      ...
      if (attempts >= 30) {
        throw new HypanelError(
          ...
          "Try stopping the server manually and try again",
          undefined,
          500
        );
      }
    ...
    try {
      ...
      try {
        ...
        if ((extractError as any).code === 'ENOENT') {
          ...
          throw new HypanelError(
            ...
            "Check disk space and permissions, then retry update",
            undefined,
            500
          );
        }
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name.endsWith("-back")) {
          ...
          try {
            ...
            const backups: Array<{ name: string; path: string; modified: Date }> = [];
  ...
  private getNextScheduledRestartTime(server: Server): number | null {
    ...
    if (freq === "every_6h") {
      ...
      for (const hour of slots) {
        ...
        if (next.getTime() <= now.getTime()) lastPassed = next.getTime();
      }
      if (lastPassed !== null) return lastPassed;
   ...
   */
  private runScheduledRestartWarnings(): void {
    let servers: Server[];
    try {
      servers = getAllServers();
    } catch (err) {
    ...
    for (const server of candidates) {
      ...
      if (server.lastRestartWarningForRunAt === nextRun) continue;
      ...
      try {
        ...
        logger.warn(
          `Failed to send 15-minute restart warning for server ${server.id} (${server.name}): ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
  }
   ...
   */
  private getNextAdvancedBackupTime(server: Server): number | null {
   ...
   */
  async runAdvancedBackup(serverId: string): Promise<void> {
    const dbServer = getServerFromDb(serverId);
    if (!dbServer) {
   ...
   */
  private cleanupOldAdvancedBackups(serverId: string): void {
    const dbServer = getServerFromDb(serverId);
    const maxCount = dbServer?.advancedBackupMaxCount ?? 1;
    const advancedDir = path.join(appConfig.backupDir, "advanced", serverId);

    if (!fs.existsSync(advancedDir)) return;

    try {
      const items = fs.readdirSync(advancedDir, { withFileTypes: true });
      const archives: Array<{ name: string; path: string; modified: Date }> = [];
   ...
   */
  private async runScheduledAdvancedBackups(): Promise<void> {
    let servers: Server[];
    try {
      servers = getAllServers();
    } catch (err) {
    ...
    for (const server of candidates) {
      ...
      try {
        ...
        logger.error(
          `Scheduled advanced backup failed for server ${server.id} (${server.name}): ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
  }

  private startAdvancedBackupScheduler(): void {
  ...
  private stopAdvancedBackupScheduler(): void {
   ...
   */
  private async pollPlayerLists(): Promise<void> {
    const onlineServers = Array.from(this.instances.entries()).filter(
      ([, instance]) => instance.getStatus() === "online"
    );
   ...
   * Stop backup cleanup
   */
  private stopBackupCleanup(): void {
