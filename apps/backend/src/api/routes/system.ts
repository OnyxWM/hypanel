    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(currentPkgPath) && fs.existsSync(newPkgPath)) {
          const currentPkg = JSON.parse(fs.readFileSync(currentPkgPath, "utf8")) as { dependencies?: Record<string, string>; optionalDependencies?: Record<string, string> };
          const newPkg = JSON.parse(fs.readFileSync(newPkgPath, "utf8")) as { dependencies?: Record<string, string>; optionalDependencies?: Record<string, string> };
